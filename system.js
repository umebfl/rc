...
export default class System extends Component {
        ...
        request.onreadystatechange = (e) => {
            ...
            if (request.status === 200) {
                ...
                for(var i = 0; i < breed.length; i++) {
                    ...
                    const current_bond = (parseInt(info[8]) * breed_item.unit * breed_item.rate).toFixed(0)
    ...
    handle_reload_log() {
        ...
        const data = this.props.context.state.breed
        let finish_count = 0
        let ft_log = []
            ...
            request.onreadystatechange = (e) => {
                ...
                if (request.status === 200) {
                    ...
                    if(finish_count === data.length) {
                        ...
                        console.log('flow', '完成日志数据的请求并写入存储', ft_log)
                        this.fetch_current_data()
                    }
                                    ...
                                    (v, k)  => (
                                        <List.Item key={k} wrap={true}>
                                            <View style={{flexDirection: 'row', flowWrap: 'wrap', fontSize: 10}}>
                                                ...
                                                {/* 当前价  */}
                                                <Text style={{width: 50, textAlign: 'right'}}>{v.info ? parseInt(v.info[8]).toFixed(0) : '-'}</Text>
                                                {/* 价格状态  */}
                                            ...
                                            <View style={{flexDirection: 'row', flowWrap: 'wrap', fontSize: 8,}}>
                                                ...
                                                {/* 保证金比例  */}
                                                <Text style={{width: 20, textAlign: 'right', color: '#999'}}>{(1 / v.rate).toFixed(0)}</Text>
                                                {/* 一手保证金  */}
